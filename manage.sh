...
retain_old_versions() {
  ...
  if [ -t 0 ]; then
    printf 'ectropy: remove previously installed versions after install? [y/N] ' >&2
  ...
  echo "ectropy: preserving previous versions; pass --retain=false to prune after install" >&2
  printf '%s' true
}
