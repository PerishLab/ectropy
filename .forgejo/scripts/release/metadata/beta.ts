const USER_AGENT = "ectropy-release-beta/1.0";
...
async function stableFloor(): Promise<string | null> {
  const publicUrl = (Deno.env.get("ECTROPY_RELEASES_PUBLIC_URL") ?? "").replace(/\/+$/, "");
  if (!publicUrl) {
    fail("ECTROPY_RELEASES_PUBLIC_URL is required");
  }
  ...
  try {
    ...
    if (!response.ok) {
      await response.body?.cancel();
      fail(`failed to fetch R2 stable metadata: HTTP ${response.status}`);
    }
    ...
    const version = metadata.stableVersion || metadata.releaseVersion;
    if (typeof version !== "string" || version === "") {
      fail("R2 stable metadata must include stableVersion or releaseVersion");
    }
    const match = /^v?(\d+\.\d+\.\d+)$/.exec(version);
    if (!match) {
      fail(`R2 stable metadata has invalid version ${version}`);
    }
    return match[1];
  } catch (error) {
...
async function nextBeta(cargo: string): Promise<[string, number, string, string]> {
  const publicUrl = (Deno.env.get("ECTROPY_RELEASES_PUBLIC_URL") ?? "").replace(/\/+$/, "");
  let metadataUrl = Deno.env.get("ECTROPY_BETA_METADATA_URL");
  if (!metadataUrl) {
    if (!publicUrl) {
      fail("ECTROPY_RELEASES_PUBLIC_URL is required");
    }
